...
if ($currentHour >= 0 && $currentHour <= 10) {
    ...
    $logOutput .= "INFO: Daytime/Evening run. Targeting today: {$targetDate}\n";
}
...
if ($httpCode === 200 && $response) {
    ...
    if (isset($apiData['status']) && $apiData['status'] === 'success') {
            ...
            function processStandardGame($pdo, $gameData, $tableName, $isPowerBall) {
                ...
                if (strpos($tableName, 'powerball') !== false) {
                    if ($dayOfWeek !== 2 && $dayOfWeek !== 5) {
                        return 0; // Skip: Not a valid PowerBall draw day
                    }
                }

                $balls = $gameData['winning_numbers'] ?? [];
                if(empty($balls)) return 0; // Skip if no numbers are published yet
        ...
        if ($lottoData) {
            ...
            $logOutput .= "Lotto: " . ($rows > 0 ? "Inserted\n" : "Skipped/Exists/InvalidDay\n");
        }

        $lottoP1Data = $apiData['results']['lotto_plus_1_results'] ?? $apiData['results']['lotto_plus_1'] ?? $apiData['results']['lotto_plus_results'] ?? null;
        if ($lottoP1Data) {
        ...
        // 3. PowerBall Games
        $pbData = $apiData['results']['powerball_results'] ?? $apiData['results']['powerball'] ?? null;
        if ($pbData) {
            $rows = processStandardGame($pdo, $pbData, 'powerball_results', true);
            $logOutput .= "PowerBall: " . ($rows > 0 ? "Inserted\n" : "Skipped/Exists/InvalidDay\n");
        }

        $pbPlusData = $apiData['results']['powerball_plus_results'] ?? $apiData['results']['powerball_plus'] ?? null;
        if ($pbPlusData) {
        ...
        $logOutput .= "API Error: " . ($apiData['message'] ?? 'Unknown Error') . "\n";
    }
} elseif ($httpCode === 400 || $httpCode === 404) {
    // GRACEFUL HANDLING: API provider hasn't published anything for today yet
    $logOutput .= "STATUS: API returned HTTP {$httpCode}.\n";
    $logOutput .= "REASON: The results for {$targetDate} are not yet published by the API provider. Checking again on next cron run.\n";
} else {
    $logOutput .= "CRITICAL ERROR: API connection failed. HTTP Code: {$httpCode}\n";
}
